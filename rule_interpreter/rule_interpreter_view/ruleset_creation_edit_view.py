...
def save_ruleset(database, user, jsonBlob):
    database.cur.execute("SELECT MAX(ID) FROM Ruleset;")
    data = database.cur.fetchone()
    if data[0] is None:
        currentIdx = 0
    else: 
        currentIdx = data[0] + 1
    database.cur.execute("insert into Ruleset values(?, ?, ?);", (currentIdx, user, jsonBlob,))
    database.conn.commit()

    database.close()

def main(rulesetName):
    newRuleset = False
    if rulesetName == "":
        newRuleset = True
    ...
    buttonrects = [pygame.Rect((50, 175, 950, 500))]
    textSizes = [(50, 125)]
    if newRuleset:
        buttonrects.append(pygame.Rect((50, 50, 950, 50)))
        textSizes.append((50, 5))
    print(buttonrects)
        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                if x in range(1050, 1200) and y in range(160, 200):
                    ...
                    save_ruleset(database, "shally", picklestring)
